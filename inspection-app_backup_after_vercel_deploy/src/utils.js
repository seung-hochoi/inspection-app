...
export const buildNormalizedRows = (parsedRows) =>
  (parsedRows || []).map((rawRow, index) => {
    ...
    return {
      ...row,
      // Canonical Korean field names — downstream code reads these keys directly
      '협력사명':     partner,
      '상품코드':     productCode,
      '상품명':       productName,
      '센터명':       center,
...
/**
 * Returns a stable client/session ID for this browser.
 * Generated once with crypto.randomUUID() and persisted in localStorage.
 * Used to distinguish "same user re-editing" from "different user conflict".
 */
export const getClientId = (() => {
  const KEY = '__inspection_client_id';
  let cached = null;
  return () => {
    ...
    try {
      ...
      if (!cached) {
        cached = typeof crypto !== 'undefined' && crypto.randomUUID
          ? crypto.randomUUID()
          : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
        localStorage.setItem(KEY, cached);
      }
    } catch (_) {
      cached = 'unknown-client';
    }
    return cached;
  };
})();
