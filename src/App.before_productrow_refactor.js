...
function App() {
  ...
  const analyticsKpis = useMemo(() => {
    ...
    const inspectedQty = (inspectionRows || []).reduce((sum, row) => sum + parseQty(row.검품수량), 0);
    const inspectedSku = new Set(
      (inspectionRows || [])
        .filter((row) => parseQty(row.검품수량) > 0)
        .map((row) => `${row.협력사명 || ""}||${row.상품코드 || ""}`)
    ).size;
    ...
    return [
      { label: "총 입고금액", value: formatDashboardValue("총 입고금액", dashboardSummary?.totalInboundAmount ?? totalInboundAmount) },
      { label: "총 입고수량", value: formatDashboardValue("총 입고수량", dashboardSummary?.totalInboundQty ?? totalInboundQty) },
      { label: "검품 입고금액", value: formatDashboardValue("검품 입고금액", dashboardSummary?.inspectionInboundAmount ?? 0) },
      { label: "검품 SKU", value: formatDashboardValue("검품 SKU", dashboardSummary?.inspectionSku ?? inspectedSku) },
      { label: "검품 수량", value: formatDashboardValue("검품 수량", dashboardSummary?.inspectionQty ?? inspectedQty) },
      { label: "검품율", value: formatDashboardValue("검품율", dashboardSummary?.inspectionRate ?? inspectionRate) },
      { label: "실 검품율", value: formatDashboardValue("실 검품율", dashboardSummary?.realInspectionRate ?? inspectionRate) },
      { label: "SKU 커버리지", value: formatDashboardValue("SKU 커버리지", dashboardSummary?.skuCoverage ?? skuCoverage) },
    ];
  ...
  return (
    <div style={styles.app}>
      ...
      {activeTab === "inspection" && (bootLoading || uploadingCsv || error || message) && (
            ...
            <div key={`${record.__rowNumber || "row"}-${index}`} style={styles.historyCard}>
              ...
              <div style={styles.cardTopRow}>
                <div style={styles.cardTitle}>
                  {String(record["상품명"] || record["?곹뭹紐?"] || "상품명 없음")}
                </div>
                <span style={styles.typeBadge}>{getRecordType(record)}</span>
              </div>
              <div style={styles.cardMeta}>코드 {String(record["상품코드"] || record["?곹뭹肄붾뱶"] || "-")}</div>
              <div style={styles.cardMeta}>센터 {String(record["센터명"] || record["?쇳꽣紐?"] || "-")}</div>
              <div style={styles.cardMeta}>협력사 {String(record["협력사명"] || record["?묐젰?щ챸"] || "-")}</div>
              <div style={styles.qtyRow}>
                <span style={styles.qtyChip}>처리수량 {getRecordQtyText(record)}</span>
                <span style={styles.qtyChip}>{formatDateTime(record["작성일시"] || record["?묒꽦?쇱떆"])}</span>
              </div>
              <div style={styles.historyMemo}>{String(record["비고"] || record["鍮꾧퀬"] || "-")}</div>
