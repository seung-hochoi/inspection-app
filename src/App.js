...
function App() {
  ...
  const [analyticsPeriod, setAnalyticsPeriod] = useState("1d");
  const [lastHappycallFileName, setLastHappycallFileName] = useState("");
  ...
  const fetchInspectionRowsData = useCallback(async () => {
    ...
    const result = await response.json();
    if (!response.ok || result.ok === false) throw new Error(result.message || "검품 데이터 불러오기 실패");
  ...
  useEffect(() => {
    loadBootstrap();
    loadInspectionRows();
  }, [loadBootstrap, loadInspectionRows]);

  useEffect(() => {
    if (activeTab === "records") {
      loadHistoryRows();
    }
  }, [activeTab, loadHistoryRows]);
  ...
  const analyticsTopList = useMemo(
    () =>
      (happycallAnalytics?.periods?.[analyticsPeriod]?.topProducts || [])
        .filter(isClassifiedHappycallProduct)
        .slice(0, 5)
        .map((item, index) => ({
          rank: index + 1,
    ...
    [happycallAnalytics, analyticsPeriod, productImageMap]
  );
  ...
  return (
    <div style={styles.app}>
        ...
        <div style={styles.headerTopRow}>
          <div style={styles.brandBlock}>
            <div style={styles.brandRow}>
              ...
              <h1 style={styles.title}>GS25 검품 PDA</h1>
            </div>
            ...
            <div style={styles.subtitle}>MADE BY SEUNGHO</div>
          </div>
          <div style={styles.headerModeBadge}>
            {activeTab === "inspection" ? "검품" : activeTab === "records" ? "내역" : "통계"}
          </div>
        </div>
      </div>
      ...
      <div style={styles.tabBar}>
        {[
          { id: "inspection", icon: "📋", label: "검품" },
          { id: "records", icon: "📄", label: "내역" },
          { id: "analytics", icon: "📊", label: "통계" },
        ].map((tab) => (
          <button
            key={tab.id}
            type="button"
            ...
            style={{
              ...styles.tabBarButton,
              ...(activeTab === tab.id ? styles.tabBarButtonActive : {}),
            }}
          >
            <span style={styles.tabBarIcon}>{tab.icon}</span>
          ...
          <div style={styles.panel}>
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <button
                ...
                style={{ ...styles.secondaryButton, flex: 1, ...(mode === "inspection" ? styles.tabButtonActive : {}) }}
              >
                🔎 검품 모드
              </button>
              <button
                ...
                style={{ ...styles.secondaryButton, flex: 1, ...(mode === "return" ? styles.tabButtonActive : {}) }}
              >
                📦 회송 모드
              </button>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button
                ...
                style={{ ...styles.secondaryButton, minHeight: 40, fontSize: 13 }}
              >
                {uploadingCsv ? "처리 중..." : "📄 CSV 업로드"}
              </button>
              <button
                type="button"
                onClick={() => happycallFileInputRef.current?.click()}
                disabled={uploadingHappycallCsv}
                style={{ ...styles.secondaryButton, minHeight: 40, fontSize: 13, opacity: uploadingHappycallCsv ? 0.7 : 1 }}
              >
                {uploadingHappycallCsv ? "처리 중..." : "🗂 해피콜"}
              </button>
              <button
                type="button"
                onClick={() => { setError(""); setImageRegisterSearch(""); setShowImageRegister(true); }}
                style={{ ...styles.secondaryButton, minHeight: 40, fontSize: 13 }}
              >
                🖼 이미지
              </button>
              <button
                type="button"
                onClick={() => { setError(""); setAdminPassword(""); setShowAdminReset(true); }}
                style={{ ...styles.secondaryButton, minHeight: 40, fontSize: 13 }}
              >
                👤 관리자
              </button>
            ...
            {currentFileName && (
              <div style={styles.metaText}>현재 작업: {currentFileName}</div>
            )}
          </div>
                        ...
                        {partnerGroup.products.map((product) => {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...styles.card,
                                ...(isCompleted ? { background: "#f0fdf4", borderColor: "#bbf7d0" } : {}),
                              }}
                            >
                              {mode === "inspection" ? (
      ...
      {activeTab === "records" && (
        <div style={styles.panel}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={styles.sectionTitle}>검품 내역</div>
            <button type="button" onClick={loadHistoryRows} style={{ ...styles.secondaryButton, minHeight: 36, fontSize: 13 }}>
              새로고침
            </button>
          </div>
          {historyLoading ? (
      ...
      {activeTab === "analytics" && (
        <>
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {[["1d", "전일"], ["7d", "7일"], ["30d", "한달"]].map(([key, label]) => (
              <button
                key={key}
                type="button"
                ...
                style={{
                  ...
                  minHeight: 40,
                  fontSize: 13,
                  ...(analyticsPeriod === key ? styles.tabButtonActive : {}),
                }}
              >
                {label}
              </button>
            ...
            <div style={styles.happycallHeader}>
              <div>
                <div style={styles.sectionTitle}>
                  해피콜 TOP 5 ({analyticsPeriod === "1d" ? "전일" : analyticsPeriod === "7d" ? "7일" : "한달"})
                </div>
                <div style={styles.heroSubtext}>
                  {analyticsPeriod === "1d" ? "전일" : analyticsPeriod === "7d" ? "최근 7일" : "최근 한달"} 접수 해피콜 기준
                </div>
              </div>
            </div>

            {analyticsTopList.length === 0 ? (
              ...
              <div style={styles.happycallShowcase}>
                  ...
                  <div style={styles.heroTopCard}>
                    <div style={styles.heroTopCopy}>
                      <div style={styles.heroTopBadge}>
                        <span style={styles.heroTopMedal}>{getTopMedal(analyticsHeroCard.rank)}</span>
                        <span style={styles.heroTopBadgeText}>TOP {analyticsHeroCard.rank}</span>
                      </div>
                      <div style={styles.heroTopName}>{analyticsHeroCard.productName}</div>
                      <div style={styles.heroTopMeta}>
                        {analyticsHeroCard.count.toLocaleString("ko-KR")}건 · {formatPercent(analyticsHeroCard.share)}
                      </div>
                        ...
                        <div style={styles.heroProgressTrack}>
                          <div
                            style={{
                              ...
                              width: `${Math.max(14, Math.min(100, analyticsHeroCard.share * 100))}%`,
                            }}
                          />
                        </div>
                        <div style={styles.heroProgressValue}>{formatPercent(analyticsHeroCard.share)}</div>
                      </div>
                    </div>
                    <div style={styles.heroImageFrame}>
                      {analyticsHeroCard.imageSrc ? (
                        <img
                          src={analyticsHeroCard.imageSrc}
                          alt={analyticsHeroCard.productName}
                ...
                {analyticsMiniCards.length ? (
                  <div style={styles.heroMiniGrid}>
                    {analyticsMiniCards.map((card) => (
                      <div
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <button
              type="button"
              onClick={() => { loadHistoryRows(); setActiveTab("records"); }}
