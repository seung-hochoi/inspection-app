...
job_${hashString( JSON.stringify( (rows || []).map((row) => ({ 상품코드: row.__productCode, 상품명: row.__productName, 센터: row.__center, 협력사: row.__partner, 수량: row.__qty, })) ) )};
...
try {
  ...
  for (const payload of tasks) {
    await saveSingleRecord(payload);
  }
  ...
  if (tasks.length > 0) {
    ...
    const result = await response.json();
    if (response.ok && result.ok !== false) {
      const reloaded = Array.isArray(result.records) ? result.records : [];
      reloaded.sort((a, b) =>
        String(b.작성일시 || "").localeCompare(String(a.작성일시 || ""), "ko")
      );
...
<input ref={fileInputRef} type="file" accept=".csv" onChange={handleCsvUpload} style={styles.hiddenInput} />
  ...
  <div style={styles.countRow}>
    <div style={styles.countText}>
      총 {groupedPartners.reduce((sum, item) => sum + item.products.length, 0)}건
    </div>
    <button
      type="button"
      onClick={async () => {
          ...
          {expandedPartner === partnerGroup.partner && (
            <div style={styles.partnerBody}>
              {partnerGroup.products.map((product) => {
                const returnExpandedKey = `${partnerGroup.partner}__${product.productCode}`;
                const isReturnOpen = expandedProductCode === returnExpandedKey;
                ...
                const returnDraftKey = `return||${product.productCode}||${selectedCenter}`;
                const inspectionDraftKey = `inspection||${product.productCode}`;
                const returnDraft = drafts[returnDraftKey] || {};
                      ...
                      <div style={styles.cardInlineInspection}>
                        ...
                        <div style={styles.inlineInspectionRow}>
                          <input
                            type="number"
                            min="0"
                            value={inspectionDraft.inspectionQty || ""}
                            onChange={(e) =>
                              updateDraft(inspectionDraftKey, "inspectionQty", e.target.value)
                            }
                          ...
                          <button
                            ...
                            disabled={savingKey === inspectionDraftKey}
                            style={styles.inlineSaveButton}
                          >
                            {savingKey === inspectionDraftKey ? "..." : "저장"}
                          </button>
                        </div>
                      </div>
                    ) : (
                        ...
                        <button
                          ...
                          onClick={() => {
                            setExpandedProductCode((prev) =>
                              prev === returnExpandedKey ? "" : returnExpandedKey
                            );
                          ...
                          <div style={styles.editorBox}>
                            ...
                            <div style={styles.grid2}>
                              <div style={styles.formGroup}>
                                ...
                                <input
                                  type="number"
                                  min="0"
                                  value={returnDraft.returnQty || ""}
                                  onChange={(e) =>
                                    updateDraft(returnDraftKey, "returnQty", e.target.value)
                                  }
                                  style={styles.input}
                                />
                              </div>
                              <div style={styles.formGroup}>
                                ...
                                <input
                                  type="number"
                                  min="0"
                                  value={returnDraft.exchangeQty || ""}
                                  onChange={(e) =>
                                    updateDraft(returnDraftKey, "exchangeQty", e.target.value)
                                  }
                                  style={styles.input}
                                />
                              </div>
                            ...
                            <div style={styles.formGroup}>
                              ...
                              <textarea
                                ...
                                onChange={(e) =>
                                  updateDraft(returnDraftKey, "memo", e.target.value)
                                }
                            ...
                            <div style={styles.formGroup}>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  updateDraft(returnDraftKey, "photoFile", file);
                                  updateDraft(returnDraftKey, "photoName", file?.name || "");
                                }}
                                style={styles.fileInput}
                              />
                              <div style={styles.metaText}>
                                {returnDraft.photoName || "선택된 사진 없음"}
                              </div>
                            </div>
                            ...
                            <button
                              ...
                              disabled={savingKey === returnDraftKey}
                              style={styles.saveButton}
                            >
                              {savingKey === returnDraftKey ? "저장 중..." : "저장"}
                            </button>
                          </div>
                        )}
                      </>
