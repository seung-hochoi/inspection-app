...
function DraftPhotoPreviewItem({ file, index, onRemove, onOpen, styles }) {
  ...
  return (
    <div style={styles.draftPhotoCard}>
      {previewUrl ? (
        <img
          src={previewUrl}
          alt={file?.fileName || file?.name || `선택 사진 ${index + 1}`}
          style={{ ...styles.draftPhotoImage, cursor: onOpen ? "zoom-in" : undefined }}
          onClick={() => onOpen && onOpen(previewUrl)}
        />
      ) : (
...
function DraftPhotoPreviewList({ files, onRemove, onOpen, styles }) {
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState("inspection"); // "inspection" | "records" | "analytics"
  const [happycallFileName, setHappycallFileName] = useState("");
  const [editingRecordKey, setEditingRecordKey] = useState("");
  const [editingDefectReason, setEditingDefectReason] = useState("");
  const [savingDefectReason, setSavingDefectReason] = useState(false);
    ...
    setPendingMap((prev) => {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.type === "inspection") {
          merged.회송수량 = parseQty(entry.회송수량) || parseQty(prevEntry.회송수량) || 0;
          merged.교환수량 = parseQty(entry.교환수량) || parseQty(prevEntry.교환수량) || 0;
          merged.센터명 = entry.센터명 || prevEntry.센터명 || "";
          merged.비고 = entry.비고 !== undefined ? entry.비고 : (prevEntry.비고 || "");
          merged.photoItems =
        ...
        if (entry.type === "movement") {
          ...
          merged.비고 = entry.비고 !== undefined ? String(entry.비고 || "") : (prevEntry.비고 || "");
  ...
  const buildInspectionPendingEntry = useCallback((product, nextDraft = {}) => {
    ...
    const centerName = nextDraft.centerName || pendingMap[entityKey]?.센터명 || "";
    const defectReason = nextDraft.defectReason !== undefined
      ? String(nextDraft.defectReason || "")
      : pendingMap[entityKey]?.비고 || "";
    ...
    return {
      ...
      회송수량: parseQty(nextDraft.returnQty) || pendingMap[entityKey]?.회송수량 || 0,
      교환수량: parseQty(nextDraft.exchangeQty) || pendingMap[entityKey]?.교환수량 || 0,
      센터명: centerName,
      비고: defectReason,
  ...
  const updateRecordDefectReason = async (record, defectReason) => {
    const rowNumber = Number(record.__rowNumber || 0);
    if (!rowNumber) {
      setError("수정할 행 정보를 찾지 못했습니다.");
      return;
    }

    try {
      setSavingDefectReason(true);
      setError("");
      const response = await fetch(SCRIPT_URL, {
        method: "POST",
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: JSON.stringify({
          action: "updateDefectReason",
          payload: { rowNumber, defectReason },
        }),
      });
      const result = await response.json();
      if (!response.ok || result.ok === false) {
        throw new Error(result.message || "불량사유 수정 실패");
      }
      if (Array.isArray(result?.records)) {
        ...
        setHistoryRows((prev) =>
          prev.map((item) =>
            Number(item.__rowNumber) === rowNumber
              ? { ...item, 비고: defectReason }
              : item
          )
        );
      }
      setEditingRecordKey("");
      setEditingDefectReason("");
      setToast("불량사유 수정 완료");
    } catch (err) {
      setError(err.message || "불량사유 수정 실패");
    } finally {
      setSavingDefectReason(false);
    }
  };
    ...
    <div style={styles.app}>
      ...
      <div style={styles.headerCard}>
        <div style={styles.headerTopRow}>
          ...
          <div style={styles.headerModeBadge}>{activeTab === "inspection" ? (mode === "inspection" ? "검품 모드" : "회송/교환 모드") : activeTab === "records" ? "내역" : "분석"}</div>
        </div>

        {/* 파일 상태 카드 */}
        <div style={styles.fileStatusRow}>
          <div style={styles.fileStatusCard}>
            <span style={styles.fileStatusIcon}>{currentFileName ? "✅" : "📄"}</span>
            <div style={styles.fileStatusInfo}>
              <div style={styles.fileStatusLabel}>검품 CSV</div>
              <div style={styles.fileStatusName}>{currentFileName || "미업로드"}</div>
            </div>
            <button type="button" onClick={() => fileInputRef.current?.click()} style={styles.fileStatusUploadBtn}>
              {uploadingCsv ? "처리중..." : "업로드"}
            </button>
          </div>
          <div style={styles.fileStatusCard}>
            <span style={styles.fileStatusIcon}>{happycallFileName ? "✅" : "🗂"}</span>
            <div style={styles.fileStatusInfo}>
              <div style={styles.fileStatusLabel}>해피콜</div>
              <div style={styles.fileStatusName}>{happycallFileName || "미업로드"}</div>
            </div>
            <button type="button" onClick={() => happycallFileInputRef.current?.click()} disabled={uploadingHappycallCsv} style={styles.fileStatusUploadBtn}>
              {uploadingHappycallCsv ? "처리중..." : "업로드"}
            </button>
          </div>
        </div>
        ...
        <div style={styles.quickActionGrid}>
          <button
            type="button"
            onClick={() => { setActiveTab("inspection"); setMode("inspection"); }}
            style={{ ...styles.quickActionCard, ...(activeTab === "inspection" && mode === "inspection" ? styles.quickActionCardActive : {}) }}
          >
          ...
          <button
            type="button"
            onClick={() => { setActiveTab("inspection"); setMode("return"); }}
            style={{ ...styles.quickActionCard, ...(activeTab === "inspection" && mode === "return" ? styles.quickActionCardActive : {}) }}
          >
            <span style={styles.quickActionIcon}>📦</span>
          ...
          <button
            ...
            style={{ ...styles.quickActionCard, ...(activeTab === "records" ? styles.quickActionCardActive : {}) }}
          >
            <span style={styles.quickActionIcon}>📄</span>
          ...
          <button
            ...
            style={{ ...styles.quickActionCard, ...(activeTab === "analytics" ? styles.quickActionCardActive : {}) }}
          >
            <span style={styles.quickActionIcon}>📊</span>
            <span style={styles.quickActionText}>분석</span>
          </button>
        </div>

        <div style={styles.csvActionRowSecondary}>
          <button
            type="button"
            onClick={() => {
              setError("");
                      ...
                      <div key={`${partnerGroup.partner}-${product.productCode}`} style={styles.card}>
                        {mode === "inspection" ? (
                          <div style={styles.cardInlineInspection}>
                            ...
                            <div style={styles.inlineInspectionRow}>
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  // Auto-save: queue entry and let debounce flush
                                  const autoEntry = buildInspectionPendingEntry(product, nextDraft);
                                  upsertPendingEntries([autoEntry]);
                                }}
                                style={{
                                  ...styles.inlineQtyInput,
                                  background: parseQty(draft.inspectionQty) > 0 ? "#d1fae5" : "#ffffff",
                                }}
                                placeholder="검품수량"
                              />
                            </div>
                            ...
                            <div style={styles.grid2}>
                              <div style={styles.formGroup}>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const nextDraft = { ...draft, returnQty: e.target.value };
                                    setDrafts((prev) => ({ ...prev, [draftKey]: nextDraft }));
                                    upsertPendingEntries([buildInspectionPendingEntry(product, nextDraft)]);
                                  }}
                                  style={{
                                    ...styles.input,
                                    background: parseQty(draft.returnQty) > 0 ? "#d1fae5" : "#ffffff",
                                  }}
                                />
                              </div>
                              <div style={styles.formGroup}>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const nextDraft = { ...draft, exchangeQty: e.target.value };
                                    setDrafts((prev) => ({ ...prev, [draftKey]: nextDraft }));
                                    upsertPendingEntries([buildInspectionPendingEntry(product, nextDraft)]);
                                  }}
                                  style={{
                                    ...styles.input,
                                    background: parseQty(draft.exchangeQty) > 0 ? "#d1fae5" : "#ffffff",
                                  }}
                                />
                              </div>
                            </div>

                            {product.centers.length > 1 && (
                              <div style={styles.formGroup}>
                                <label style={styles.label}>센터 선택</label>
                                <select
                                  value={draft.centerName || ""}
                                  onChange={(e) => {
                                    const nextDraft = { ...draft, centerName: e.target.value };
                                    setDrafts((prev) => ({ ...prev, [draftKey]: nextDraft }));
                                    upsertPendingEntries([buildInspectionPendingEntry(product, nextDraft)]);
                                  }}
                                  style={styles.input}
                            ...
                            <div style={styles.formGroup}>
                              <label style={styles.label}>불량사유</label>
                              <textarea
                                value={draft.defectReason || ""}
                                onChange={(e) => {
                                  const nextDraft = { ...draft, defectReason: e.target.value };
                                  setDrafts((prev) => ({ ...prev, [draftKey]: nextDraft }));
                                  upsertPendingEntries([buildInspectionPendingEntry(product, nextDraft)]);
                                }}
      ...
      {activeTab === "records" && (
        <div style={styles.panel}>
          <div style={styles.sheetHeader}>
            <div style={styles.sectionTitle}>저장 내역</div>
            <button
              type="button"
              onClick={() => loadHistoryRows()}
              style={styles.secondaryButton}
          ...
          {(error || message) && (
            <div style={error ? styles.errorBox : styles.infoBox}>{error || message}</div>
          )}
          {historyLoading ? (
            ...
            <div style={styles.sheetList}>
              {historyRows.map((record, index) => {
                const recordKey = `${record.__rowNumber || "row"}-${record.작성일시 || "time"}-${index}`;
                  ...
                  >
                    ...
                    {isEditing ? (
                      <div style={styles.formGroup}>
                        <label style={styles.label}>불량사유</label>
                        <textarea
                          value={editingDefectReason}
                          onChange={(e) => setEditingDefectReason(e.target.value)}
                          style={styles.textarea}
                          rows={2}
                          placeholder="불량 사유 입력"
                        />
                        <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                          <button
                            type="button"
                            onClick={() => updateRecordDefectReason(record, editingDefectReason)}
                            disabled={savingDefectReason}
                            style={{ ...styles.saveButton, flex: 1, minHeight: 38, fontSize: 14, marginTop: 0 }}
                          >
                            {savingDefectReason ? "저장중..." : "수정완료"}
                          </button>
                          <button
                            type="button"
                            onClick={() => { setEditingRecordKey(""); setEditingDefectReason(""); }}
                            style={{ ...styles.secondaryButton, flex: 1, minHeight: 38 }}
                          >
                            취소
                          </button>
                        </div>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                        <div style={{ ...styles.historyMemo, flex: 1 }}>
                          <span style={styles.label}>불량사유: </span>{record.비고 || "-"}
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            setEditingRecordKey(recordKey);
                            setEditingDefectReason(record.비고 || "");
                          }}
                          style={{ ...styles.secondaryButton, padding: "4px 10px", fontSize: 12, minHeight: 28 }}
                        >
                          수정
                        </button>
                      </div>
      ...
      {activeTab === "analytics" && (
        <div>
          ...
          <div style={styles.panel}>
            <div style={styles.sectionTitle}>분석</div>
            <div style={styles.metaText}>현재 작업: {currentFileName || "업로드된 파일 없음"}</div>
            <div style={styles.metaText}>파일 수정일자: {currentFileModifiedAt ? formatDateTime(currentFileModifiedAt) : "-"}</div>
          </div>

          <div style={styles.panel}>
            <div style={styles.happycallHeader}>
              <div>
                <div style={styles.sectionTitle}>{selectedHappycallPeriodMeta.title}</div>
                <div style={styles.heroSubtext}>{selectedHappycallPeriodMeta.subtitle}</div>
              </div>
              <div style={styles.happycallPeriodRow}>
                <button
            ...
            {selectedHappycallTopList.length === 0 ? (
              <div style={styles.emptyBox}>해피콜 데이터가 없습니다. 해피콜 CSV를 업로드하세요.</div>
            ) : (
              <div>
                {selectedHappycallTopList.map((card) => (
                  <div
                    key={`analytics-top-${card.rank}`}
                    style={{
                      ...styles.heroMiniCard,
                      marginBottom: 10,
                      borderColor: card.rank === 1 ? "#fca5a5" : card.rank === 2 ? "#93c5fd" : card.rank === 3 ? "#86efac" : "#dbe3f0",
                    }}
                  >
                    <div style={styles.heroMiniLabel}>
                      <span>{getTopMedal(card.rank) || "•"}</span>
              ...
              <div style={styles.sheetList}>
                {historyRows.map((record, index) => (
                  ...
                  >
                    ...
                    <div style={styles.historyMemo}><span style={styles.label}>불량사유: </span>{record.비고 || "-"}</div>
