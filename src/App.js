...
function App() {
  ...
  const saveReturnExchange = async (product, centerName) => {
    ...
    if (returnQty <= 0 && exchangeQty <= 0 && !defectReason && photoFiles.length === 0) {
  ...
  const handleHappycallCsvUpload = async (e) => {
    ...
    try {
      ...
      setHappycallUploadedAt(new Date().toISOString());
      setMessage("");
      setToast(
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={styles.statusCardRow}>
        <div style={{ ...styles.statusCard, ...(currentFileName ? styles.statusCardActive : {}) }}>
          <div style={styles.statusCardIcon}>{currentFileName ? "✅" : "📁"}</div>
          <div style={styles.statusCardContent}>
            <div style={styles.statusCardLabel}>검품 CSV</div>
            <div style={styles.statusCardValue}>{currentFileName || "업로드 필요"}</div>
            {currentFileModifiedAt && (
              <div style={styles.statusCardMeta}>{formatDateTime(currentFileModifiedAt)}</div>
            )}
          </div>
        </div>
        <div style={{ ...styles.statusCard, ...(happycallFileName ? styles.statusCardActive : {}) }}>
          <div style={styles.statusCardIcon}>{happycallFileName ? "✅" : "📊"}</div>
          <div style={styles.statusCardContent}>
            <div style={styles.statusCardLabel}>해피콜 파일</div>
            <div style={styles.statusCardValue}>{happycallFileName || "업로드 필요"}</div>
            {happycallUploadedAt && (
              <div style={styles.statusCardMeta}>{formatDateTime(happycallUploadedAt)}</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={styles.headerCard}>
        ...
        <div style={styles.tabRow}>
          ...
          <button
            type="button"
            onClick={() => { setActiveTab("records"); loadHistoryRows(); }}
                            ...
                            {mode === "inspection" ? (
                              <div style={styles.cardInlineInspection}>
                                ...
                                <div style={styles.inlineInspectionRow}>
                                  <input
                                    ...
                                    style={{
                                      ...
                                      background: parseQty(draft.inspectionQty) > 0 ? "#dcfce7" : "#ffffff",
                                      borderColor: parseQty(draft.inspectionQty) > 0 ? "#16a34a" : "#d1d5db",
                                      color: parseQty(draft.inspectionQty) > 0 ? "#15803d" : "#1f2937",
                                    }}
                                    placeholder="검품수량"
                                  />
                                </div>
                                <div style={styles.formGroup}>
                                  ...
                                  />
                                  {Array.isArray(draft.photoFiles) && draft.photoFiles.length > 0 ? (
                                    <div style={styles.draftPhotoRow}>
                                      {draft.photoFiles.map((file, idx) => {
                                        const url = file._previewUrl || (file instanceof File ? URL.createObjectURL(file) : null);
                                        return url ? (
                                          <img
                                            key={idx}
                                            src={url}
                                            alt={`검품 사진 ${idx + 1}`}
                                            style={styles.draftPhotoThumb}
                                    ...
                                    <div style={styles.metaText}>선택된 사진 없음</div>
                                  )}
                                </div>
                              </div>
                            ) : (
                                  ...
                                  <div style={styles.editorBox}>
                                    <div style={styles.formGroup}>
                                      ...
                                      <textarea
                                        ...
                                        onChange={(e) => updateDraft(draftKey, "defectReason", e.target.value)}
                                        style={styles.textarea}
                                        rows={3}
                                        placeholder="불량 사유 직접 입력"
                                      />
                                    </div>
                                    ...
                                    <div style={styles.grid2}>
                                      <div style={styles.formGroup}>
                                        ...
                                        <input
                                          ...
                                          style={{
                                            ...styles.input,
                                            background: parseQty(draft.returnQty) > 0 ? "#dcfce7" : "#ffffff",
                                            borderColor: parseQty(draft.returnQty) > 0 ? "#16a34a" : "#d1d5db",
                                          }}
                                        />
                                      </div>
                                      <div style={styles.formGroup}>
                                        ...
                                        <input
                                          ...
                                          style={{
                                            ...styles.input,
                                            background: parseQty(draft.exchangeQty) > 0 ? "#dcfce7" : "#ffffff",
                                            borderColor: parseQty(draft.exchangeQty) > 0 ? "#16a34a" : "#d1d5db",
                                          }}
                                        />
                                      </div>
                                    </div>
                                    ...
                                    >
                                      {actionStatus === "saving" ? "저장 중..." : "수정 완료"}
                                    </button>
                                  </div>
                                )}
                              </>
        ...
        <div>
          <div style={styles.panel}>
            <div style={styles.csvHeaderRow}>
              ...
              <button type="button" onClick={loadHistoryRows} style={styles.secondaryButton}>
                새로고침
              </button>
            </div>
          </div>
          ...
          {historyLoading ? (
            ...
            <div style={styles.list}>
              {historyRows.map((record, index) => (
                <div
                  key={`${record.__rowNumber || "row"}-${index}`}
                ...
                >
                  ...
                  {record.비고 && (
                    <div style={styles.historyMemo}>불량사유: {record.비고}</div>
                  )}
                  ...
                  <div style={styles.recordActions}>
                    <button
                      type="button"
                      onClick={() => {
                        setRecordEditId(Number(record.__rowNumber));
                        setRecordEditDraft({
                          returnQty: String(parseQty(record.회송수량) || ""),
                          exchangeQty: String(parseQty(record.교환수량) || ""),
                          defectReason: record.비고 || "",
                        });
                      }}
                      style={styles.secondaryButton}
                    >
                    ...
                    >
                      {deletingRowNumber === Number(record.__rowNumber) ? "..." : "삭제"}
                    </button>
                  </div>
                </div>
              ))}
          ...
          {recordEditId !== null && (
            <div style={styles.sheetOverlay} onClick={() => !recordSaving && setRecordEditId(null)}>
              <div style={styles.bottomSheet} onClick={(e) => e.stopPropagation()}>
                <div style={styles.sheetHandle} />
                <div style={styles.sheetHeader}>
                  <h2 style={styles.sheetTitle}>내역 수정</h2>
                  <button
                    type="button"
                    onClick={() => !recordSaving && setRecordEditId(null)}
                ...
                <div style={styles.formGroup}>
                  <label style={styles.label}>불량사유</label>
                  <textarea
                    value={recordEditDraft.defectReason || ""}
                    onChange={(e) =>
                      setRecordEditDraft((prev) => ({ ...prev, defectReason: e.target.value }))
                    }
                    style={styles.textarea}
                    rows={3}
                    placeholder="불량 사유 직접 입력"
                ...
                <div style={styles.grid2}>
                  <div style={styles.formGroup}>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={recordEditDraft.returnQty || ""}
                      onChange={(e) =>
                        setRecordEditDraft((prev) => ({ ...prev, returnQty: e.target.value }))
                      }
                      style={styles.input}
                      disabled={recordSaving}
                    />
                  ...
                  <div style={styles.formGroup}>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={recordEditDraft.exchangeQty || ""}
                      onChange={(e) =>
                        setRecordEditDraft((prev) => ({ ...prev, exchangeQty: e.target.value }))
                      }
                      style={styles.input}
                      disabled={recordSaving}
                    />
                    ...
                    try {
                      ...
                      const response = await fetch(SCRIPT_URL, {
                        ...
                        body: JSON.stringify({
                          action: "updateRecord",
                          payload: {
                            rowNumber: recordEditId,
                            회송수량: parseQty(recordEditDraft.returnQty),
                            교환수량: parseQty(recordEditDraft.exchangeQty),
                            비고: recordEditDraft.defectReason || "",
                          },
                        }),
                      });
                      const result = await response.json();
                      ...
                      setHistoryRows((prev) =>
                        prev.map((r) =>
                          Number(r.__rowNumber) === recordEditId
                            ? {
                                ...r,
                                회송수량: parseQty(recordEditDraft.returnQty),
                                교환수량: parseQty(recordEditDraft.exchangeQty),
      ...
      {activeTab === "analytics" && (
        <div>
          <div style={styles.panel}>
            <div style={styles.sectionTitle}>통계</div>
            <div style={styles.tabRow}>
              {[
                { key: "1d", label: "일별" },
                { key: "7d", label: "주별" },
                ...
                <button
                  ...
                  style={{
                    ...styles.tabButton,
                    ...(selectedPeriod === p.key ? styles.tabButtonActive : {}),
                  }}
                >
                  {p.label}
                </button>
          ...
          <div style={styles.kpiGrid}>
            {(() => {
              const periodData = happycallAnalytics?.periods?.[selectedPeriod] || {};
              return [
                { label: "해피콜 총 건수", value: (periodData.totalCount || 0).toLocaleString("ko-KR") + "건" },
                { label: "TOP 상품 수", value: (periodData.topProducts?.length || 0) + "개" },
                { label: "주요 불량 유형", value: periodData.topReasons?.[0]?.reason || "-" },
                { label: "분석 데이터", value: happycallFileName || (happycallAnalytics?.lastUpdated ? "로드됨" : "없음") },
              ].map((kpi, i) => (
          ...
          <div style={styles.analyticsGrid}>
            <div style={styles.analyticsLeftPanel}>
              <div style={styles.panel}>
                <div style={styles.sectionTitle}>불량 유형 TOP 5</div>
                {(happycallAnalytics?.periods?.[selectedPeriod]?.topReasons || []).length === 0 ? (
                  <div style={styles.emptyBox}>데이터가 없습니다.</div>
                ) : (
                  (happycallAnalytics?.periods?.[selectedPeriod]?.topReasons || []).slice(0, 5).map((item, i) => (
                    <div key={i} style={styles.analyticsRankRow}>
                      <span style={styles.analyticsRankNum}>{i + 1}</span>
                      <span style={styles.analyticsRankLabel}>{item.reason || "-"}</span>
                      <span style={styles.analyticsRankValue}>{(item.count || 0).toLocaleString("ko-KR")}건</span>
                    </div>
                  ))
                )}
              </div>
            ...
            <div style={styles.analyticsRightPanel}>
              <div style={styles.panel}>
                <div style={styles.sectionTitle}>
                  해피콜 TOP 5
                  <span style={styles.periodBadge}>
                    {selectedPeriod === "1d" ? "전일" : selectedPeriod === "7d" ? "최근 7일" : "최근 1개월"}
                  </span>
                </div>
                {(happycallAnalytics?.periods?.[selectedPeriod]?.topProducts || []).length === 0 ? (
                  <div style={styles.emptyBox}>
                    {happycallFileName ? "해당 기간 데이터가 없습니다." : "해피콜 파일을 업로드해 주세요."}
                  </div>
                ) : (
                  (happycallAnalytics?.periods?.[selectedPeriod]?.topProducts || []).slice(0, 5).map((item, index) => {
                    const rank = index + 1;
                    return (
                      <div
                        key={`analytics-top-${rank}`}
                        style={{
                          ...styles.analyticsTopCard,
                          borderLeft: `4px solid ${rank === 1 ? "#ef4444" : rank === 2 ? "#3b82f6" : rank === 3 ? "#22c55e" : "#e5e7eb"}`,
                        }}
                      >
                        <div style={styles.analyticsTopRankRow}>
                          <span style={styles.analyticsTopMedal}>{getTopMedal(rank) || `#${rank}`}</span>
                          <span style={{ ...styles.analyticsTopBadge, ...getHappycallRankStyle(rank) }}>
                            TOP.{rank}
                          </span>
                        </div>
                        <div style={styles.analyticsTopName}>{item.productName || "-"}</div>
                        <div style={styles.analyticsTopMeta}>
                          {(item.count || 0).toLocaleString("ko-KR")}건 · {formatPercent(item.share)}
                        </div>
                      </div>
                    );
                  })
