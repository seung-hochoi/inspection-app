...
import { Search, ScanLine, ClipboardList, Upload, ArrowUp, ArrowDown, ArrowUpDown, X } from 'lucide-react';
...
export default function InspectionPage({
  ...
  const deduplicatedRows = useMemo(() => {
    ...
    for (const r of activeRows) {
      ...
      if (map.has(key)) {
        ...
        map.set(key, { ...prev, '발주수량': total, '전체발주수량': total, __qty: total });
      } else {
        map.set(key, { ...r });
      }
    }
  ...
  // Recompute filter only when partners, filter mode, or draft state change
  const filteredPartners = useMemo(
    () => applyFilter(partners, filter, jobKey, drafts),
    [partners, filter, jobKey, drafts],
  );
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            onClick={toggleSort}
            title={sortQty === 'asc' ? '발주 오름차순' : sortQty === 'desc' ? '발주 내림차순' : '발주수량 정렬'}
            style={{
              height: 32, padding: '0 10px', flexShrink: 0,
              display: 'flex', alignItems: 'center', gap: 4,
              background: sortQty ? C.primaryLight : C.bgAlt,
              color: sortQty ? C.primary : C.muted,
              border: `1px solid ${sortQty ? C.primaryMid : C.border}`,
              borderRadius: radius.sm,
              fontSize: 11.5, fontWeight: sortQty ? 700 : 500,
              cursor: 'pointer', fontFamily: font.base, transition: trans,
            }}
          >
            {sortQty === 'asc'
          ...
          {sortQty && (
            <button
              ...
              style={{
                ...
                background: C.bgAlt, color: C.muted2,
