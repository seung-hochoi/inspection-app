...
export default function ReferenceFunctionalApp(props) {
  ...
  const isBusy = uploadingCsv || uploadingHappycallCsv;
  const statusTitle = error ? "처리 오류" : isBusy ? "업로드 진행 중" : "작업 상태";
  ...
  return (
    <div style={{ minHeight: "100vh", background: colors.bg, padding: 16, color: colors.text }}>
      <div style={{ maxWidth: 1480, margin: "0 auto" }}>
        ...
        {(isBusy || statusText) ? (
          <div style={{ background: "#fff", border: `1px solid ${error ? "#f2b8b8" : colors.border}`, borderRadius: 18, padding: 14, boxShadow: colors.shadow, marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 10 }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: error ? colors.red : colors.text }}>{statusTitle}</div>
              {currentFileName ? <div style={{ fontSize: 12, color: colors.muted }}>{currentFileName}</div> : null}
            </div>
            <div style={{ height: 10, borderRadius: 999, background: "#edf2f8", overflow: "hidden", border: `1px solid ${colors.border}` }}>
              <div style={{ width: isBusy ? "45%" : "100%", height: "100%", borderRadius: 999, background: error ? "#f08c8c" : "linear-gradient(90deg, #2f6df6 0%, #68a4ff 100%)" }} />
            </div>
            <div style={{ marginTop: 10, fontSize: 14, color: error ? colors.red : colors.muted }}>{statusText}</div>
          </div>
        ) : null}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div style={{ display: "grid", gap: 16 }}>
                ...
                <div style={{ border: `1px solid ${colors.border}`, borderRadius: 18, background: "#fff", padding: 16, boxShadow: colors.shadow }}>
                  ...
                  <button type="button" onClick={onFlushPending} style={{ width: "100%", height: 50, borderRadius: 12, border: `1px solid ${colors.blue}`, background: "#fff", color: colors.blue, fontSize: 16, fontWeight: 800 }}>저장(일괄)</button>
                </div>
              </div>
              <div style={{ display: "grid", gap: 16 }}>
                <div style={{ border: `1px solid ${colors.border}`, borderRadius: 18, background: "#fff", padding: 16, boxShadow: colors.shadow }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                    ...
                    <button type="button" onClick={onDownloadMovementZip} style={zipButtonStyle}>불량사진 저장</button>
                    <button type="button" onClick={onDownloadMovementZip} style={zipButtonStyle}>당도사진 저장</button>
                    <button type="button" onClick={onDownloadMovementZip} style={zipButtonStyle}>중량사진 저장</button>
                  </div>
                ...
                <div style={{ border: `1px solid ${colors.border}`, borderRadius: 18, background: "#fff", padding: 16, boxShadow: colors.shadow }}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                    {[["1d", "일별"], ["7d", "주별"], ["30d", "월별"]].map(([key, label]) => (
                      <button key={key} type="button" onClick={() => onSelectPeriod(key)} style={{ height: 34, padding: "0 13px", borderRadius: 10, border: selectedHappycallPeriod === key ? "1px solid #a9ccff" : `1px solid ${colors.border}`, background: selectedHappycallPeriod === key ? colors.blueSoft : "#fff", color: selectedHappycallPeriod === key ? colors.blue : colors.muted, fontSize: 13, fontWeight: 700 }}>{label}</button>
                    ))}
                  </div>
                  <div style={{ display: "grid", gap: 10 }}>
                    {[happycallHeroCard, ...happycallMiniCards].filter(Boolean).map((entry) => {
                      const medalColor = entry.rank === 1 ? "#c9971a" : entry.rank === 2 ? "#73839b" : entry.rank === 3 ? "#ad6b3b" : colors.text;
                      const medalBg = entry.rank === 1 ? "#fff4cf" : entry.rank === 2 ? "#eef3f8" : entry.rank === 3 ? "#f8eadf" : "#fbfdff";
                      return (
                        <div key={`top-${entry.rank}-${entry.productCode || entry.productName}`} style={{ border: `1px solid ${colors.border}`, borderRadius: 14, background: "#fbfdff", padding: "12px 14px", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ minWidth: 62, height: 30, padding: "0 10px", borderRadius: 999, background: medalBg, color: medalColor, display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 800 }}>{`TOP.${entry.rank}`}</span>
                          <span style={{ flex: 1 }}>{entry.productName}{entry.partnerName ? `(${entry.partnerName})` : ""}</span>
                          <span style={{ fontWeight: 800 }}>{formatCount(entry.count)}</span>
                        </div>
                      );
                    })}
                  </div>
